...
DataPath.ICT =  ['D:\OneDrive - The Pennsylvania State University\Projects\ACT-America\Data\ACTAMERICA_LARC_Merge\'] ; % Or any other path to netCDF files 

Dir.Out     =  [codepath 'Out_2019\'] ;
mkdir(Dir.Out)
...
% Flag to decide whether to use ICARTT
Flag_ICARTT = true 
...
if strcmp(plane,'b200')    
    ...
    if Flag_ICARTT
        f = dir([DataPath.ICT 'ACTAMERICA-mrg05-b200_merge_' daystr '*.ict']);
    else
        f = dir([DataPath.NC 'ACTAMERICA-mrg05-b200_merge_' daystr '*.nc']);
    end
    if ~isempty(f)
        if Flag_ICARTT
            NC = readICART_ORNL([DataPath.ICT f.name], Vars); % a bit dumb that I called the return NC, but ...
        else
...
elseif strcmp(plane,'c130')
    Vars =  {'UTC', 'LATITUDE','LONGITUDE', 'GPS_ALT','THETA', 'Altitude-AGL_GoogleMaps', 'HDG'...
             'TEMPERATURE', 'TotalAirTemp', 'ManeuverFlag'};
    if Flag_ICARTT
       f = dir([DataPath.ICT 'ACTAMERICA-mrg05-c130_merge_' daystr '*.ict']);
    else
       f = dir([DataPath.NC 'ACTAMERICA-mrg05-c130_merge_' daystr '*.nc']);
    end
    if ~isempty(f)
        if Flag_ICARTT
            NC = readICART_ORNL([DataPath.ICT f.name], Vars); 
        else
            info = ncinfo([DataPath.NC f.name]);
            NC = readNC_ORNL([DataPath.NC f.name], Vars);
        end
...
% account for fact that ICART heights are in km
if max([NC.Altitude_AGL_GoogleMaps])<100
    for i =1:length(NC)
       NC(i).Altitude_AGL_GoogleMaps = NC(i).Altitude_AGL_GoogleMaps*1000;
    end
end
if max([NC.GPS_ALT])<100
    for i =1:length(NC)
       NC(i).GPS_ALT= NC(i).GPS_ALT*1000;
    end
end
...
if strcmp(plane,'b200')
    ...
    elseif day == datenum('2018-05-12','yyyy-mm-dd')
        MF_QC(1001:1110) = 0;
    elseif day == datenum('2019-06-11','yyyy-mm-dd')
        ind = find(MF==7 & UTC<=18.98);
        MF(ind)    = 5; % split landing leg into spiral and landing
        ind = find(UTC> 19.305 & UTC<=19.318 & MF ==6);
        MF(ind)    = 1; % extend take-off leg
        ind = find(UTC> 19.6 & UTC<=19.622 & MF ==6);
        MF(ind)    = 7; % missing landing leg
    elseif day == datenum('2019-06-17','yyyy-mm-dd')
        ind = find(UTC> 20.0 & UTC<=20.3 & MF ==7);
        MF(ind)    = 6; % shorten landing leg
    elseif day == datenum('2019-06-26','yyyy-mm-dd')
        %ind = find(UTC> 21.5 & UTC<=22.0 & MF ==2);
        %MF(ind)    = 4; % spiral identification
        %ind = find(UTC> 21.5 & UTC<=22.5 & MF ==5);
        %MF(ind)    = 5; % spiral identification
    elseif day == datenum('2019-06-26','yyyy-mm-dd')
        %ind = find(UTC> 15 & UTC<=17 & MF ==5);
    elseif day == datenum('2019-07-01','yyyy-mm-dd')
        ind = find(UTC> 20.1 & UTC<=20.186 & MF ==6);
        MF(ind)     = 1; % extend takeoff leg
        MF_QC(ind)  = 0;
    elseif day == datenum('2019-07-09','yyyy-mm-dd')
       ind = find(UTC> 17.81 & UTC<=18.2 & MF ==3); 
       MF(ind)     = 6; % extend level leg
       MF_QC(UTC> 18.05 & UTC<=18.15)  = 0;
    elseif day == datenum('2019-07-10','yyyy-mm-dd')   
       ind = find(UTC> 19.0 & UTC<= 19.46 & MF ==6);
       MF(ind)     = 1; % extend take-off 
    elseif day == datenum('2019-07-19','yyyy-mm-dd')   
       ind = find(UTC> 16.15 & UTC<= 16.307 & MF ==6);
       MF(ind)     = 2; % extend climb  
       ind = find(UTC> 16.307 & UTC<= 16.36 & MF ==6);
       MF(ind)     = 3; % extend descent to match
       MF_QC(UTC> 16.145 & UTC<=16.404)  = 0; %adjust QC flag
       ind = find(UTC> 20.3 & UTC<= 20.7 & ~(MF ==6))
       MF(ind)  = 6; % low quality level leg, QC is already low
    elseif day == datenum('2019-07-22','yyyy-mm-dd') 
       MF_QC(UTC> 17.1 & UTC<=17.55 & MF==2)  = 0; %adjust QC flag
    elseif day == datenum('2019-07-27','yyyy-mm-dd') 
       ind = find(UTC> 18.9 & UTC<= 18.985 & MF ==6);    
        MF(ind)  = 1; % extend takeoff
    end
elseif strcmp(plane,'c130')
    ...
    elseif day == datenum('2018-05-20','yyyy-mm-dd')
         MF(1749:1824)    = 3;
    elseif day == datenum('2019-06-11','yyyy-mm-dd')
        %ind = find(UTC> 19.2 & UTC<=19.35 & MF ==3);
        %MF(ind)    = 5; % change to sprial
    elseif day == datenum('2019-06-17','yyyy-mm-dd')
        ind = find(UTC> 17.41 & UTC<=17.433 & MF ==2);
        MF(ind)    = 5; % shorten landing leg
    elseif day == datenum('2019-06-27','yyyy-mm-dd')
        ind = find(UTC> 21.0 & UTC<=21.36 & MF ==7);
        MF(ind)    = 6; % shorten landing leg
    elseif day == datenum('2019-07-01','yyyy-mm-dd')
       ind = find(UTC> 21.0 & UTC<=21.255 & MF ==7); 
       MF(ind)    = 3; % split landing leg into leg-down and landing
    elseif day == datenum('2019-07-09','yyyy-mm-dd')
       ind = find(UTC> 16.9 & UTC<=16.93 & MF ==2); 
       MF(ind)     = 5; % change descent/ascent boundary
       MF_QC(UTC> 16.844 & UTC<=16.93)  = 0;
    elseif day == datenum('2019-07-10','yyyy-mm-dd')    
       ind = find(UTC> 21.7 & UTC<=21.9 & MF ==3); % half turn
       ind = find(UTC> 22.2 & UTC<=22.46 & MF ==7);
       MF(ind)     = 5; % split landing into landing and spiral
    elseif day == datenum('2019-07-11','yyyy-mm-dd')   
       ind = find(UTC> 15.88 & UTC<= 16.0 & MF ==2);
       MF(ind)     = 6; % extend level leg  
       MF_QC(UTC> 15.88 & UTC<= 16.0 )  = 0;
    elseif day == datenum('2019-07-19','yyyy-mm-dd')  
       ind = find(UTC> 19.06 & UTC<= 19.24 & ~(MF ==6));
       MF(ind)     = 6; % overpass over hill, make low QC leg
       MF_QC(UTC> 19.06 & UTC<= 19.24 )  = 0;
    elseif day == datenum('2019-07-20','yyyy-mm-dd') 
       ind = find(UTC> 17.4 & UTC<= 17.46 & MF ==6);
       MF(ind)     = 3; % extend downward 
       MF_QC(UTC> 16.95 & UTC<= 17.46 )  = 0;
    elseif day == datenum('2019-07-24','yyyy-mm-dd') 
       ind = find(UTC> 17.9 & UTC<= 18.1 & MF ==3);
       MF(ind)     = 6; % make low QC level leg
       MF_QC(ind)  = 0;
    elseif day == datenum('2019-07-25','yyyy-mm-dd') 
       ind = find(UTC> 20.3 & UTC<= 20.59 & MF ==7);
       MF(ind)     = 6; % make low QC level leg
    end
end
...
if ishandle(fig)
    ...
    saveas(fig, [Dir.Out 'ALTvMF' daystr '.fig']) 
    close(fig)
end 
end
end
